...
ORDER BY 
    sumqty DESC";	
		...
		#showing record 
		 ...
		 $topsale_q = "SELECT sum(Quantity) AS sumqty, order_detail.Product_ID,Product_Name FROM order_detail, product, order_r WHERE order_detail.Product_ID = product.Product_ID AND order_r.Order_ID = order_detail.Order_ID GROUP BY product.Product_ID ORDER BY sum(Quantity) DESC";	
		$query = mysqli_query($connection,$topsale_q);
		}
		else if (!empty($Todate) && !empty($Fromdate)) {
			echo $topsale_q = "SELECT 
    od.Product_ID ,sum(od.Quantity) AS sumqty 
FROM 
    order_detail od
    LEFT JOIN product p ON od.Product_ID = p.Product_ID
...
ORDER BY 
    sumqty DESC";	
    echo "Showing Records Between ".$Fromdate." And ".$Todate;
			$query = mysqli_query($connection,$topsale_q);
		}
